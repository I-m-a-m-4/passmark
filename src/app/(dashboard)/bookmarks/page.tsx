...
export default function BookmarksPage() {
  ...
  const removeBookmark = async (id: string) => {
    ...
    const newBookmarks = userData.bookmarkedMaterials.filter(
      (bid: string) => bid !== id,
    );
    try {
      ...
      toast({
        ...
        description: "The material has been removed from your collection.",
      });
    } catch (e) {
      toast({
        variant: "destructive",
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-zinc-500 dark:text-gray-500">
          Your curated collection of academic knowledge nodes.
        </p>
      </div>
          ...
          {savedMaterials.map((q) => {
            const isUnlocked =
              (userData?.unlockedParts || []).includes(
                `${q.university}_${q.department}_${q.level}`,
              ) || userData?.role === "admin";
            return (
                ...
                <div className="p-6 flex flex-col md:flex-row md:items-center gap-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className="font-bold text-lg uppercase tracking-tight text-zinc-950 dark:text-white">
                        {q.courseCode}
                      </h4>
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center space-y-6 bg-black/[0.02] dark:bg-white/[0.02] rounded-[3rem] border border-dashed border-black/10 dark:border-white/10">
          ...
          <div>
            ...
            <p className="text-zinc-500 dark:text-gray-500 max-w-xs mx-auto text-sm leading-relaxed italic">
              Immutable links to your most studied materials will appear here.
              Pin a question from the dashboard to start your collection.
            </p>
          </div>
          <Button
            className="font-bold bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl px-10 h-12 shadow-xl shadow-emerald-500/20"
            asChild
          >
