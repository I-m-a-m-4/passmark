      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold font-headline tracking-tight">
            Campus Reps Management
          </h1>
          <p className="text-muted-foreground">
            Monitor and manage the "door-to-door" protocol agents.
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-card/50 backdrop-blur-sm border-white/5">
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-emerald-500 font-bold mt-1 tracking-wider uppercase">
              Protocol Active
            </p>
          </CardContent>
        </Card>
        <Card className="bg-card/50 backdrop-blur-sm border-white/5">
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-muted-foreground font-bold mt-1 tracking-wider uppercase">
      ...
      <div className="grid gap-4">
        {filteredReps.length > 0 ? (
          filteredReps.map((rep) => (
            <Card
              key={rep.id}
              className="group hover:border-emerald-500/30 transition-all bg-card/50 backdrop-blur-sm border-white/5 overflow-hidden"
            >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground font-medium">
                    <span className="flex items-center gap-1.5">
                      <MapPin className="w-3.5 h-3.5" /> {rep.university}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <ShieldCheck className="w-3.5 h-3.5" /> Code:{" "}
                      <b className="text-foreground">{rep.referralCode}</b>
                    </span>
          ...
          <div className="text-center py-20 bg-card/20 rounded-[2rem] border-2 border-dashed border-white/5">
            <p className="text-muted-foreground">
              No campus reps found in the registry.
            </p>
          </div>
        )}
      </div>
