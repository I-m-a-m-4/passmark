...
import { Search as SearchIcon, Filter, Sparkles, BookOpen, GraduationCap, Download, Bookmark, FileText, Lock, Clock } from "lucide-react";
...
export default function SearchPage() {
    ...
    const performSearch = async (s: string) => {
        ...
        try {
            ...
            const allItems = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as any[];
    ...
    const toggleBookmark = async (item: any) => {
        ...
        try {
            await updateDoc(doc(db, "users", auth.currentUser.uid), { bookmarkedMaterials: newBookmarks });
            setUserData({ ...userData, bookmarkedMaterials: newBookmarks });
            toast({ title: isBookmarked ? "Removed" : "Saved", description: item.courseCode + " updated." });
        } catch (e) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col gap-2">
                <h1 className="text-3xl font-bold font-headline tracking-tight text-zinc-900 dark:text-white">Library Search</h1>
                <p className="text-zinc-500 dark:text-gray-500">Find any study material across our global university network.</p>
            </div>
            ...
            <div className="space-y-6">
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {results.map((item) => {
                            const isBookmarked = (userData?.bookmarkedMaterials || []).includes(item.id);
                            const isUnlocked = (userData?.unlockedParts || []).includes(`${item.university}_${item.department}_${item.level}`) || userData?.role === "admin";
                            
                            return (
                                <AuraCard key={item.id} className="group hover:scale-[1.002] transition-transform">
                                    <div className="p-6 flex flex-col md:flex-row md:items-center gap-6">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-1">
                                                <h4 className="font-bold text-lg uppercase tracking-tight text-zinc-950 dark:text-white truncate">{item.courseCode}</h4>
                                                <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-zinc-100 dark:bg-white/10 text-zinc-500">{item.year}</span>
                                                <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-500">{item.type}</span>
                                            </div>
                                            <p className="text-sm text-zinc-500 dark:text-gray-400 font-medium truncate">{item.courseTitle}</p>
                                            <p className="text-[9px] text-zinc-400 uppercase font-bold tracking-widest mt-1">{item.university}</p>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Button 
                                                variant="ghost" 
                                                size="icon" 
                                                className={cn("h-10 w-10 rounded-full", isBookmarked ? "text-emerald-500 bg-emerald-500/10" : "text-zinc-400")}
                    ...
                    <div className="text-center py-20 bg-black/[0.02] dark:bg-white/[0.02] rounded-[3rem] border border-dashed border-black/10 dark:border-white/10">
                        ...
                        <h3 className="font-bold text-xl mb-2 text-zinc-400 dark:text-gray-400 italic">No matching results...</h3>
                        <p className="text-zinc-500 dark:text-gray-500 text-sm">Try searching by a different course code or university name.</p>
                    </div>
                ) : (
                    <div className="text-center py-24 bg-black/[0.02] dark:bg-white/[0.02] rounded-[3rem] border border-dashed border-black/10 dark:border-white/10">
                        ...
                        <h3 className="font-bold text-xl mb-2 text-zinc-400 dark:text-gray-400 italic">Awaiting search signal...</h3>
                        <p className="text-zinc-500 dark:text-gray-500 text-sm">Enter a course code or topic above to explore the network.</p>
                    </div>
                )}
            </div>
            ...
            {!results.length && (
                <AuraCard className="bg-emerald-500/5 border-emerald-500/10 p-12 text-center max-w-3xl mx-auto shadow-2xl rounded-[3rem]">
                    ...
                    <h2 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-white font-headline">Smart indexing active...</h2>
                    <p className="text-zinc-500 dark:text-gray-500 text-sm max-w-lg mx-auto leading-relaxed italic">
                        Our university material database is currently being updated with new papers from across the network. Full semantic search will be available shortly.
                    </p>
                </AuraCard>
            )}
        </div>
